...
int elems_to_args(Elements *elements, DocoptArgs *args, bool help,
    ...
    for (i=0; i < elements->n_arguments; i++) {
        ...
        if (!strcmp(argument->name, "<cetacean_file>")) {
            args->cetacean_file = argument->value;
        } else if (!strcmp(argument->name, "<observer_locations_file>")) {
            args->observer_locations_file = argument->value;
        }
    }
    return 0;
}
...
DocoptArgs docopt(int argc, char *argv[], bool help, const char *version) {
    ...
    Argument arguments[] = {
        {"<cetacean_file>", NULL, NULL},
        {"<observer_locations_file>", NULL, NULL}
    };
    Option options[] = {
    };
